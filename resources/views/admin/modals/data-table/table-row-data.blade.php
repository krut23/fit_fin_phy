<td class="align-middle text-nowrap" ng-repeat="(key, field) in dataTableFields"
    ...
    <ANY ng-switch="field.displayType">
        ...
        <!-- Custom table field -->
        @include('admin.modals.data-table.table-row-data-custom')
        ...
        <ANY ng-switch-when="image">
            <ANY ng-if="row[key]">
                ...
                {{--                <img class="@{{field.className}}" ng-show="field.className" alt="icon" ng-src="@{{row[key]}}" ng-click="showImagePreview(row[key])"/>--}}
                {{--                <img class="avatar avatar-lg" ng-hide="field.className" alt="icon" ng-src="@{{row[key]}}" ng-click="showImagePreview(row[key])"/>--}}
